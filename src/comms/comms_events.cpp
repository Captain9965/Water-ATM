...
EventQueue<outMessage_t, COMMS_EVENTS_QUEUE_SIZE>* CommsOutQueue::get_instance(){
    static EventQueue<outMessage_t, COMMS_EVENTS_QUEUE_SIZE> queue;
    return &queue;
}
...
comms_ev_error_t publish_check_event(check_event_t * event){
    ...
    if(!CommsOutQueue::get_instance()->enqueue(msg)){
